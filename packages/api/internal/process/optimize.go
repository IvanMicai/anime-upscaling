...
func RunOptimize(ctx context.Context, cfg config.Config, d *docker.Docker, fileList []string, onEvent func(logger.JobLog), onProgress func(docker.Progress)) error {
	for i, f := range fileList {
		...
		OptimizeFile(ctx, cfg, d, f, i+1, onEvent, safeProgress(onProgress))
	}
	return nil
}

// OptimizeFile compresses a single file from input/ to optimized/ using FFmpeg.
func OptimizeFile(ctx context.Context, cfg config.Config, d *docker.Docker, filename string, index int, onEvent func(logger.JobLog), onProgress func(docker.Progress)) {
